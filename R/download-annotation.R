...
{
  dir.create(paste0('03-annotation/',species[i]))
  ...
  for(j in 1:min(c(20,N.accession)))
  {
    ...
    {
      write.csv(full.annotation,paste0('03-annotation/',species[i],'/',paste(current.accession[1:min(5,N.chromosomes)],collapse = ':'),'.csv'),row.names = F)
    }
  }
  print(i)
}
